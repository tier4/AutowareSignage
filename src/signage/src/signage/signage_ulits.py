...
def process_current_task(current_task_details, task):
    current_task_details["departure_station"] = split_name(task.get("origin_point_name", DEFAULT_DEPARTURE_NAME))
    current_task_details["arrival_station"] = split_name(task.get("destination_point_name", DEFAULT_ARRIVAL_NAME))
    try:
...
def create_next_station_list(self, current_task_details, seperated_task_list, call_type, schedule_type= ""):
    # TODO: need to add current_task_details["arrival_station"]?
    # station_list = [current_task_details["arrival_station"]]
    station_list = []

    for task in seperated_task_list["todo_list"]:
        station_list.append(task.get("destination_point_name", DEFAULT_ARRIVAL_NAME))
    ...
    # TODO: check whether can auto assign without return
    station_list = auto_add_empty_list(station_list)
